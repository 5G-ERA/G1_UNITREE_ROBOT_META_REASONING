...
FLOOR_MIN = 0.14      # fraccion minima de suelo en el centro; bajo -> camara solo bloquea si esta muy tapado
...
def floor_free_bands(img):
    """Suelo = pixeles de BAJA SATURACION (moqueta gris S~5). Devuelve la fraccion de suelo en la
    mitad inferior de cada banda (izq/centro/dcha) y refS (diag)."""
    import numpy as np
    hsv = np.asarray(img.convert("HSV")).astype(int)   # H,S,V en 0..255
    H, W = hsv.shape[:2]
    S = hsv[:, :, 1]
    floor = S < SAT_FLOOR_MAX
    refS = float(np.median(S[int(H * 0.78):, int(W * 0.30):int(W * 0.70)]))
    lo = int(H * 0.45)                                  # mitad inferior (lo cercano)

    def frac(c1, c2):
        return float(floor[lo:, int(W * c1):int(W * c2)].mean())

    return frac(0.0, 0.35), frac(0.35, 0.65), frac(0.65, 1.0), 0.0, refS
...
def save_crash_image(cdp, n, x, y, yaw, c0):
    ...
    try:
        ...
        with vlock:
            ...
            vlb = vision.get("label", ""); vblk = vision.get("block", False)
        with open(base + ".txt", "w") as f:
            ...
            f.write(f"CAMARA suelo: izq={lf:.2f} centro={cf:.2f} dcha={rf:.2f}  block={vblk} label='{vlb}'\n")
...
def yolo_worker():
    ...
    print("VISION activa (suelo" + (" + YOLO" if model else "") + ").")
    last = -1; smooth = []
    while True:
        with vlock:
        ...
        try:
            ...
            W, H = img.size
            blk = False; lbl = ""
            # 1) suelo por color (con suavizado temporal de 4 frames)
            lf0, cf0, rf0, refH, refS = floor_free_bands(img)
            ...
            if model is not None:
                res = model.predict(img, imgsz=416, conf=0.30, verbose=False)[0]
                for bx in res.boxes:
                    ...
                    if (furn and 0.18 < cx < 0.82 and area > 0.05) or \
                       (0.25 < cx < 0.75 and (area > 0.12 or (bh > 0.45 and bottom > 0.6))):
                        blk = True; lbl = name; break
            with vlock:
                vision["block"] = blk; vision["label"] = lbl; vision["side"] = side
...
def cmd_vsee():
    ...
    try:
        while True:
            ...
            with vlock:
                lf = vision.get("lf", 0); cf = vision.get("cf", 0); rf = vision.get("rf", 0)
                blk = vision["block"]; lbl = vision["label"]
                refH = vision.get("refH", 0); refS = vision.get("refS", 0)
            bar = "BLOCK(" + lbl + ")" if blk else "libre"
            print(f"  suelo  izq={lf:.2f}  centro={cf:.2f}  dcha={rf:.2f}   refHS=({refH:.0f},{refS:.0f})  -> {bar}")
            time.sleep(0.5)
          ...
          if(d && d.type==='newMap' && d.data && d.data.directOutput!=null){
            ...
            if(od){
              ...
              for(var j=0;j+2<a.length;j+=3){
                ...
                var c = (g[key]||0)+1; g[key] = c<60?c:60;
              }
            }
          }
        }catch(e){} });
...
EXP_FWD_GOOD = 0.90    # gira hasta abrir el frente DE VERDAD (giros mas pronunciados, sale de rincones)
...
def cmd_explore(secs):
    ...
    try:
        while time.time() - t0 < secs:
            ...
            if vision_on:
                if now - vcam_t > 0.5:                       # captura un frame (mismo CDP, hilo principal)
                    ...
                    vcam_t = now
                with vlock:
                    if vision["block"] and now - vision["ts"] < 1.5:
                        vb = True; vlbl = vision["label"]; vside = vision["side"]
            if cmd is None and vb:                           # override: girar hacia donde hay mas suelo
            ...
            if cmd is not None:
                ...
                if c0 > EXP_FWD_MIN:
                    ...
                    state = "ESC"; esc_t0 = time.time(); scan_t = 0; esc_dir = 0   # nuevo episodio -> re-elige lado
            if cmd is None and state == "ESC":
                if c0 > EXP_FWD_GOOD:                         # hueco real delante -> avanza
                    ...
                    if esc_dir == 0 or time.time() - scan_t > 2.5:   # elige lado UNA vez (re-evalua solo si atascado)
                        ...
                        best_off = max(WIDE_OFFS, key=lambda o: ws[o]); best_clr = ws[best_off]
                        esc_dir = -AV_TURN if best_off >= 0 else AV_TURN   # COMMIT: gira a lo mas abierto y no titubea
                        scan_t = time.time()
            ...
            line = (f"t={time.time()-t0:5.1f}/{secs:.0f} {ph} pos=({x:+.2f},{y:+.2f}) yaw={yaw:+6.1f} "
                    f"c0={sc(c0)} rear={sc(rear)} vis={vlbl if vb else '-'} best={best_off:+d}({sc(best_clr)}) "
        ...
        cdp.eval(STOP_JS); time.sleep(0.3); cdp.eval(STOP_JS)
        print("STOP. Fin de exploracion."); lg.write("FIN\n"); lg.close()
