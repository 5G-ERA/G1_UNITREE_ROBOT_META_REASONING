...
SNIFF_JS = r"""(function(){
  if(!window.__navSniff){ window.__navSniff=1; window.__navlog=[];
    var S = RTCDataChannel.prototype.send;
    RTCDataChannel.prototype.send = function(d){
      try{ var s = (typeof d==='string')? d : '';
        if(s && (s.indexOf('slam_operate')>=0 || s.indexOf('targetPose')>=0 ||
                 ...
                 s.indexOf('1102')>=0 || s.indexOf('api_id')>=0)){
          window.__navlog.push(s.slice(0,3000));
          if(window.__navlog.length>50) window.__navlog.shift();
        }
      }catch(e){}
      return S.apply(this, arguments);
    };
...
def cmd_sniffnav():
    """SNIFFER del nav nativo: captura los mensajes que la app envia por el datachannel cuando pones un
    GOAL de navegacion en su mapa (slam_operate / anyPointNavigation 1102 / obstacles_avoid). Para saber
    el formato EXACTO y replicarlo en v3. USO: lanzar esto, poner un destino en la app, y copiar lo que
    salga. (No mueve el robot; solo escucha.)"""
    cdp = get_cdp()
    cdp.eval(SNIFF_JS)
    print(">>> SNIFF NAV instalado. Ahora, EN LA APP, pon un punto de destino de navegacion en el mapa.")
    print("    Capturare el/los mensaje(s) que envie. Ctrl+C para salir.\n")
    callog = os.path.join(os.path.dirname(LOGPATH), "navsniff.log")
    seen = 0
    try:
        while True:
            try:
                s = cdp.eval("JSON.stringify(window.__navlog||[])")
            ...
            while seen < len(arr):
                ...
                print(f"\n=== MENSAJE NAV #{seen} ===\n{msg}\n")
                try:
                    with open(callog, "a") as f:
                        f.write(f"\n=== {time.strftime('%H:%M:%S')} ===\n{msg}\n")
                except Exception:
                    pass
            time.sleep(0.4)
    except KeyboardInterrupt:
        print(f"\nFin sniffnav. Guardado en {callog}")
            ...
            if cmd is not None:
                ...
                if True:
                    ...
                    if blocked and tgt is not None:
                        h0 = math.radians(yaw)
                        bd = c0 if lidar_block else 0.55             # distancia: laser real, o ~0.55m si lo ve solo la camara
                        for dd in (max(0.5, bd), bd + 0.2):
                            oc = (round((x + dd * math.cos(h0)) / OCELL), round((y + dd * math.sin(h0)) / OCELL))
                            if lidar_block:
                                omap[oc] = now; oset.add(oc)        # LiDAR real -> mapa persistente
                            else:
                                cobs[oc] = now + COBS_TTL            # camara -> capa decayente (si fuera falso, se borra)
                        oset |= {c for c, ex in cobs.items() if ex > now}
                        plan_pts = []                                # fuerza replan con el obstaculo ya marcado
