...
def yolo_worker():
    ...
    while True:
        ...
        try:
                ...
                for bx in res.boxes:
                    ...
                    if (furn and 0.18 < cx < 0.82 and area > 0.05) or \
                       (0.25 < cx < 0.75 and (area > 0.12 or (bh > 0.45 and bottom > 0.6))):
                        ...
                        # 'close' NO desde la caja YOLO (daba falsos: silla/cama a 7-13m marcadas CERCA).
                        # La cercania la decide la distancia METRICA del suelo (dmet) mas abajo.
                        break
            ...
            if dmet < CLOSE_M:                            # el suelo confirma que hay algo CERCA -> close
                close = True
            with vlock:
...
def cmd_frontier(secs, vshare=None, lock=None, stop_event=None):
    ...
    try:
        while time.time() - t0 < secs and not (stop_event is not None and stop_event.is_set()):
            ...
            # NO escribe en el mapa persistente: la camara tiene falsos positivos (edge) y amurallaba al robot.
            if cmd is None and vb and vclose:            # camara ve obstaculo CERCA -> PREFIERE GIRAR (reactivo)
