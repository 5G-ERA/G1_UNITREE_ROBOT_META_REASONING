...
def navigate_to(cdp, lg, wx, wy, label, vshare=None, lock=None, stop_event=None):
    ...
    GOV = ("DCE" + (("-" + os.environ["G1_META_ABLATE"]) if os.environ.get("G1_META_ABLATE") else "")) \
        if os.environ.get("G1_META") == "1" else "default"   # gobernanza de ESTE run (queda en log + dataset)
    ...
    minc0 = 9.9
    rd = RunRecorder("ours", label, (wx, wy)); rd.rec["governance"] = GOV   # DCE / DCE-<ablation> / default
